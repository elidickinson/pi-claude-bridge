...
- **Add: plan-aware long-context configuration (Issue #24)** — new `provider.plan` (default `"pro"`) and `provider.longContextExtraUsage` replace the always-on 1M behavior. By default models register and run at 200K; `plan: "max"` sends `[1m]` for Opus models and registers matching 1M windows; `longContextExtraUsage: true` explicitly sends `[1m]` for all 1M-capable models and registers matching 1M windows. This keeps pi's status bar and auto-compaction threshold aligned with the runtime window, avoiding the issue #24/#17/#18 "pi shows headroom but Claude Code fails around 200K" class. Provider-registered model names append `1M` after long-context config is resolved, so pi's picker/status reflects the active context window without changing the Claude Code model id policy for 200K models.
- **Bump: dependencies** — pi dev and peer dependencies to 0.79+ for the current exported `compact()` API used by the bridge-owned compaction path. Also updated Claude Agent SDK within the 0.2 line, TypeBox 1.3, pi dev packages 0.79.9, tsx 4.22, and Node 24 type definitions.
