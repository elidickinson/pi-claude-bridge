...
- **Fix: rate-limit lifecycle, session rebuild on error, and strict type checking** — clear stale rate-limit rejection markers on query completion, mark sessions for rebuild on error results, verify working directory match before session reuse, guard hot-path debug serialization, terminate child processes immediately on AskClaude aborts, and enable full strict typechecking.
