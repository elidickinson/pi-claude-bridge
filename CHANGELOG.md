...
- **Fix: `resolveModel` prefers an exact model id over an earlier id that contains it** — the single `=== || includes` pass handed an exact id to whichever listed id contained it, so once a successor such as `claude-fable-5-1` is listed ahead of `claude-fable-5`, asking AskClaude for the full id `claude-fable-5` silently ran the successor. Exact match now wins; shortcuts like `opus` and `fable` still take the first partial match.
