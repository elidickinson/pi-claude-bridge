...
- **Fix: rate-limit rejection state leak and session directory checks** — clear stale rate-limit rejection markers on query completion, verify working directory match before reusing sessions, guard hot-path tool result stringification when debug logging is off, and terminate child processes immediately on AskClaude aborts.
