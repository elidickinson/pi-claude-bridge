...
- **Split adaptive-thinking models into `-thinking` / `-instant` variants** — Opus 4.6/4.7 and Sonnet 4.6 each appear twice in the picker, suffixed `-thinking` (renders reasoning blocks) and `-instant` (no reasoning blocks; effort still applies to compute). Pi's `reasoning` slider sets effort independently, so combinations like "instant + high effort" or "thinking + low effort" are now expressible — they couldn't be with a single knob. The `-instant` variant emits `--thinking disabled` so the CC binary doesn't silently re-enable reasoning from `~/.claude/settings.json` (`alwaysThinkingEnabled` / `effortLevel`). The `-thinking` variant is listed first, so `opus`/`sonnet` fuzzy shortcuts resolve there — preserving pre-variant default UX. Haiku 4.5 is unchanged (not adaptive-thinking; uses budget-based thinking gated by `reasoning`).
- **Per-model effort mapping with full top-tier access** — replaced the single global `REASONING_TO_EFFORT` table with per-model `thinkingLevelMap` entries on each variant, read at request time. Opus 4.6/4.7 expose all 5 Anthropic effort tiers (`low/medium/high/xhigh/max`) by shifting pi's labels down one slot (`minimal→low, low→medium, …, xhigh→max`) — labels are off-by-one but `max` is finally accessible. Sonnet 4.6 has only 4 tiers (no `xhigh`) so it uses natural label-aligned mapping (`xhigh→max`). Selector also hides `off`/`minimal` on adaptive variants to match the supported set; the picker shows exactly what the model accepts. See README "Provider" section for the full mapping table.
