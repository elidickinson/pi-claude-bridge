...
- **Fix: subagents didn't resolve against the prompt pi actually queried with** — pi's system prompt widens after `before_agent_start`, as verbose MCP tool descriptions merge into it only once their servers connect. So the prompt a tool like pi-subagents embeds into a child via `ctx.getSystemPrompt()` at dispatch is larger than what `before_agent_start` recorded (measured 23,479 vs 10,988 chars, diverging at the tool list), and the child found no matching capture. The bridge now also records `ctx.getSystemPrompt()` at `agent_start` — the fully-widened prompt, captured just before the query — so a subagent embedding it resolves against it. Covered by `tests/unit-agent-start-capture.mjs`.
