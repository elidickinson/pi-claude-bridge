...
- **Tests: contract test pins the git-snapshot gate** — `tests/int-cc-contracts.mjs` gains a test that captures CC's actual request body via a local mock endpoint (request bodies aren't observable through the SDK) and asserts `CLAUDE_CODE_DISABLE_GIT_INSTRUCTIONS=1` omits the git-status snapshot from the claude_code preset system prompt. A CLI bump that restores the snapshot now fails loudly in one named test.

- **Fix: git-status snapshot no longer breaks the prompt cache between turns** — the bridge spawns a fresh Claude Code subprocess per query, and CC recomputed its environment snapshot (git status, branch, recent commits) into the system prompt on every spawn. Any working-tree change between turns silently invalidated the whole cached prefix. `CLAUDE_CODE_DISABLE_GIT_INSTRUCTIONS=1` is now set on every CC child (alongside the existing guards in `CC_CHILD_ENV`), which omits the snapshot entirely; verified byte-identical system prompts across processes against SDK 0.2.141 / CLI 2.1.141.
- **Add: `provider.excludeDynamicSections` config option** — strips CC's per-session dynamic sections (working directory, git status) from the preset system prompt; CC re-injects them as a `<system-reminder>` on the first user message instead. Off by default. Note: this alone does **not** improve cache behavior under the bridge — the re-injected reminder is restamped by every fresh subprocess anyway. It is only useful combined with the git-snapshot guard above, or for fleet deployments that need a fully static system prefix.
