...
- **Fix: a Claude CLI that exits before reading stdin crashed pi with an uncaught EPIPE** — reproducing with `pathToClaudeCodeExecutable: /bin/false`, the SDK writes the first message to the dead child's stdin; the `^0.2.141` SDK has no `error` listener on stdin, so that `write EPIPE` became an uncaughtException and pi exited 1 instead of failing the turn. The bridge now owns `spawnClaudeCodeProcess` and absorbs stdin errors, so the turn ends with `stopReason: error` and the SDK's own message (`Claude Code process exited with code 1`, or `terminated by signal SIGTERM` when the child had closed stdin but kept running and the bridge had to stop it). SDK 0.3.208 fixed the crash itself (anthropics/claude-agent-sdk-typescript#318); the guard is harmless there and can go when the dependency moves past it. Debugging side effect: with a custom spawn the SDK no longer adds its `DEBUG_CLAUDE_AGENT_SDK`-driven `--debug-file`, and CLI stderr captured for that flag goes to the `CLAUDE_BRIDGE_DEBUG` log instead of the SDK's debug file, so it is dropped unless `CLAUDE_BRIDGE_DEBUG` is also on; `CLAUDE_BRIDGE_DEBUG` itself is unchanged.
