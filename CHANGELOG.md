...
- **Fix: label 1M model display names** — provider-registered model names now append `1M` after `provider.plan` and `provider.longContextExtraUsage` are resolved, so pi's picker/status reflects the active context window without changing the Claude Code model id policy.
- **Fix: make pi compaction safe for claude-bridge (issue #18)** — the bridge now takes over `session_before_compact` for claude-bridge models, calls pi's exported `compact()` with isolated Claude Code summary subprocesses, and avoids routing concurrent split-turn summaries through the live provider's shared stream state. This prevents the `/compact` hang where one summary overwrote another stream before it received `done`. If takeover fails, compaction is cancelled with a user notification rather than falling back to the known-buggy native path. The same change preserves cumulative `<read-files>`/`<modified-files>` across extension-provided compactions by re-injecting prior compaction file ops, and adds defensive stale-`activeQuery` clearing plus overwrite logging around terminal stream events.
- **Fix: preserve shared Claude Code sessions across compaction/history rewrites (issue #25)** — shorter synthetic contexts are no longer treated as continuations of the cached main Claude Code session: they start clean, preserve the main session, and delete any temporary captured session instead of adopting it. After pi-side history rewrites such as `/compact` or `session_tree`, the next provider turn rebuilds the Claude Code JSONL in place with the original session UUID instead of resuming stale pre-rewrite history or rotating unnecessarily.
- **Add: plan-aware long-context configuration** — new `provider.plan` (default `"pro"`) and `provider.longContextExtraUsage` replace the always-on 1M behavior. By default models register and run at 200K; `plan: "max"` registers bare Opus models at 1M without appending `[1m]` because Claude Code auto-upgrades them on Max/Team Premium/Enterprise pay-as-you-go/API; `longContextExtraUsage: true` explicitly sends `[1m]` for all 1M-capable models and registers matching 1M windows. This keeps pi's status bar and auto-compaction threshold aligned with the runtime window, avoiding the issue #24/#17 "pi shows headroom but Claude Code fails around 200K" class while also avoiding the reported `opus[1m]` credit-gate risk on Max 5x (claude-code#39841). Display names are computed after config resolution so 1M registrations can be labeled accurately.
- **Tests: expand compaction, session, and context coverage** — added real-provider RPC tests for manual compaction health, split-turn compaction, second-compaction file-op carry-forward, and real `threshold` auto-compaction. Added unit coverage for long-context model registration/CLI-id policy and shorter synthetic session contexts, and hardened the post-compact session integration test to require an in-place rebuild with the original session UUID.
- **Docs: document compaction and long-context behavior** — updated the README's 1M context guidance and expanded `eli/unreleased-walkthrough.html` into a self-contained branch walkthrough with inline diagrams, highlighted snippets, rationale/tradeoff notes, and testing evidence.
- **Bump: pi dev and peer dependencies to 0.79+** — required for the current exported `compact()` API used by the bridge-owned compaction path.
- **Add: `provider.systemPromptMode` config knob** — `"append" | "replace" | false`. `"append"` (default) sends CC's `claude_code` preset with AGENTS.md and skills appended, identical to 0.4.0. `"replace"` makes the bridge act like pi's own Sonnet backend: pi's full `context.systemPrompt` is passed through verbatim and CC settings/CLAUDE.md loading is opted out (`settingSources: []`). `false` sends an empty system prompt and opts out of CC settings. The deprecated `appendSystemPrompt` boolean is preserved unchanged for backward compatibility — its `false` value still maps to the 0.4.0 "preset only" path, not to the new `false` mode (so existing configs are not silently flipped).
- **Fix: `systemPromptMode: false` no longer silently sends a ~6k character prompt** — extracts were computed unconditionally in `streamClaudeAgentSdk` and `promptAndWait`, then joined and sent as a string when mode was `false`. The doc comment claimed "no system prompt at all" but the implementation sent AGENTS.md + skills. Now extracts are gated on mode; `false` produces `systemPrompt: ""`. Same fix applies to the AskClaude path.
- **Internal: extract `resolveSystemPromptConfig()` helper** — the per-mode prompt + `settingSources` assembly is now in `src/system-prompt.ts` and shared between the main provider path and AskClaude. Adds `includeAgentsMd` / `includeSkills` options so AskClaude can skip the AGENTS.md walk and respect the per-call `appendSkills` toggle.
