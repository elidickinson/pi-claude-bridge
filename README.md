...
- `extraModels` — array of model ids to offer before pi-ai's catalog lists them, e.g. `["claude-opus-5-5"]` on pi 0.87.0. Each copies the newest catalog model of its family (limits, thinking levels) under its own id. Ignored once pi-ai ships the id, so it is safe to leave in place. The model still needs a Claude Code that serves it (the bundled SDK, or `pathToClaudeCodeExecutable`).
