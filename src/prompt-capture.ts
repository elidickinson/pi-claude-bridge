...
export function sharedPromptCaptures(limit?: number, onDiagnose?: (diagnostic: PromptCaptureDiagnostic) => void): PromptCaptures {
	const globals = globalThis as Record<symbol, PromptCaptures | undefined>;
	return (globals[SHARED_CAPTURES_KEY] ??= new PromptCaptures(limit, onDiagnose));
}
 ...
 * cut. A prompt carrying neither layer is not one `buildSystemPrompt` assembled,
 * and is left alone.
 */
